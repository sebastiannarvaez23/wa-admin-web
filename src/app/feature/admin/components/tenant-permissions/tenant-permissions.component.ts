...
@Component({
    selector: 'wa-tenant-permissions',
    templateUrl: './tenant-permissions.component.html',
    styleUrls: ['../admin-shared.css', '../admin-permissions/admin-permissions.component.css'],
})
export class TenantPermissionsComponent implements OnInit {
    ...
    constructor(private svc: AdminTenantsService) {}
    ...
    toggleRoleActive(row: RolePermissionRow, event: Event): void {
        ...
        this.svc.updateRole(row.role.id, { is_active: newActive }).subscribe({
            next: updated => {
